...
export const fetchProdRating = async (id, userId) => {
    const { data } = await guestInstance.get(`rating/product/${id}/${userId}`)
    return data
}
...
/*
 * Создание, обновление и удаление информации о товаре
 */
export const createInfo = async (productId, info) => {
    const { data } = await authInstance.post(`product/${productId}/info/create`, info)
    return data
}

export const updateInfo = async (productId, id, info) => {
    const { data } = await authInstance.put(`product/${productId}/info/update/${id}`, info)
    return data
}

export const deleteInfo = async (productId, id) => {
    const { data } = await authInstance.delete(`product/${productId}/info/delete/${id}`)
    return data
}
