...
class Order {
    ...
    async getOne(id, userId = null) {
        const options = {
            where: {id},
            include: [
                {model: OrderItemMapping, as: 'items', attributes: ['id', 'name', 'price', 'quantity', 'size_product']},
            ],
        }
        if (userId) options.where.userId = userId
        const order = await OrderMapping.findOne(options)
        if (!order) {
            throw new Error('The order was not found in the database')
        }
        return order
    }
    ...
    async create(data) {
        ...
        const created = await OrderMapping.findByPk(order.id, {
            include: [
                {model: OrderItemMapping, as: 'items', attributes: ['name', 'price', 'quantity', 'size_product']},
            ],
        })
        return created
    }
    ...
    async delete(id) {
        let order = await OrderMapping.findByPk(id, {
            include: [
                {model: OrderItemMapping, attributes: ['name', 'price', 'quantity', 'size_product']},
            ],
        })
        if (!order) {
            throw new Error('The order was not found in the database')
